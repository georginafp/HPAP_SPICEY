...
# ATAC ----
merged_df <- readRDS("/homes/users/gfuentes/scratch/projects/spicey_old/CYT/data/DA_ATAC_CTRL.rds")
...
names(retsi_gr) <- NULL
saveRDS(retsi_gr, "/homes/users/gfuentes/scratch/projects/spicey_old/CYT/data/CTRL_FINAL_ATAC.rds")


# RNA ----
gr_list <- readRDS("/homes/users/gfuentes/scratch/projects/spicey_old/CYT/data/DA_RNA_CTRL.rds")
...
getsi_gr <- imap(gr_list, ~ {
  mcols(.x)$cell_type <- .y  # .x = GRanges, .y = name
  .x
}) %>%
  purrr::reduce(c) #214896

saveRDS(getsi_gr, "/homes/users/gfuentes/scratch/projects/spicey_old/CYT/data/CTRL_FINAL_RNA.rds")
